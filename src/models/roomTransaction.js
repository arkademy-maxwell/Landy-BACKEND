...
module.exports = {
  getRoomTransaction: () => {
    return new Promise((resolve, reject) => {
      conn.query(
        `SELECT room_transaction.id, room_transaction.invoices, room_transaction.date_transaction,room_transaction.durations,room.price, room.room,room.address, room.quantity,users.title,users.first_name,users.last_name,users.phone_number,users.email,transaction.transaction FROM room_transaction LEFT JOIN room ON room_transaction.room_id = room.id LEFT JOIN users on room_transaction.users_id= users.id LEFT JOIN transaction on room_transaction.transaction_id= transaction.id`,
        (err, result) => {
          if (!err) {
            resolve(result);
          } else {
